...
def initialize_publisher():
    can0 = rospy.Publisher('can0_sent_messages', Frame, queue_size=10)
    can1 = rospy.Publisher('can1_sent_messages', Frame, queue_size=10)
...
def translate_remote_controls(ser):
    ...
    if (ser.in_waiting > 0):
        ...
        if inputJson != None and inputJson[0] == '{': #123 = '{' ascii code
...
def main():
    ...
    #-------------------ROS initialisation-------------------#
    rospy.init_node('Remote_RC_control_node', anonymous=True)
    ...
    while not rospy.is_shutdown():
        (speed,
         ...
         control,
         direction,
         mode) = user_control_loop               (speed=speed,steering=steering,braking=braking,control=control,direction=direction,	                radio_control=radio_control,serial_connection=serial_connection)

        rospy.loginfo("Speed: " + str(speed) + "     Steering: " + str(steering) +  "     Braking: " + str(braking) +"    Mode: " + str(mode))

        max_speed = 127
        #-------------------CAN 0 control-------------------#
        sendCanAcceleratorMSG(pub=pub_can0, maxSpeed=max_speed, speed=speed, steering=steering, enable=mode)#enable_acel
